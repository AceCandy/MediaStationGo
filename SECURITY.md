...
MediaStationGo 是自托管媒体系统，常部署在 NAS、家庭网络、Docker 和反向代理环境中。安全问题通常会同时涉及应用代码、容器配置、路径映射、第三方 API 凭据和外部访问入口。请按本策略报告和处理安全问题。
...
如果问题需要更长时间修复，例如涉及数据迁移、权限模型或第三方 API，我们会在私密报告中同步进展。

## 安全问题范围

欢迎报告：

- 未授权访问管理接口、媒体库、外部服务配置或用户数据。
- 普通用户越权执行管理员操作。
- 读取或写入容器可访问范围外的文件。
...
- 不要在 Issue、PR、截图或日志中公开第三方服务 Cookie、API Key 或其他凭据。
