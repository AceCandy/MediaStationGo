...
- `fetchAndSaveTMDbEpisodeDetails` 已能写入标题、简介、评分、年份、演职员和可选 still，但当前没有独立任务稳定遍历历史 Episode。
...
- TMDb 返回的非空标题、简介、评分等与当前值不同时允许覆盖；该任务是明确的 TMDb 同步入口，可能覆盖手工编辑的对应字段。
- 候选仅限标题、简介或 still 图片至少一项缺失的 Episode；三个字段均完整的 Episode 不进入任务。
...
- [ ] 任务中心出现独立的“TMDb 集信息补全/复查”，默认关闭、24 小时周期并可手动执行。
- [ ] 候选只包含有 media 且标题、简介或 still 缺失的 Episode；完整 Episode 不请求 TMDb。
- [ ] 空标题和生成标题均属于缺失；正常真实标题不单独触发任务。
- [ ] 一次执行分批处理完启动时的全部候选，不保存跨任务游标；成功请求后的 Episode 在 3 天内不再命中。
- [ ] 每个候选最多调用一次 Episode 详情接口，并可写入约定字段、演职员和 still。
- [ ] 无变化不产生逐条详情；更新、无数据、并发跳过和失败可定位到 Series/Season/Episode 与 TMDb ID。
- [ ] Episode 不再进入“TMDb 无图复查”，但本地 still 丢失仍可由“TMDb 图片本地化修复”恢复。
- [ ] 现有首次刮削、手工重刮和 Movie/Series/Season 图片任务行为保持不变。

## Out of Scope

- 不扫描没有 media 的 Episode。
- 不周期刷新标题、简介和 still 均完整的 Episode。
