...
- `TMDbRecheckPassBoundary(ctx)` returns a database timestamp after local reconciliation. `ClaimTMDbRecheckSeason(ctx, cutoff)` leases a Season; `ClaimTMDbRecheckSeasonPage(ctx, lease, cutoff)` leases up to 200 targets. `CountTMDbRechecksDue(ctx, cutoff)` counts queue rows only, including outstanding leases.
...
- Due claims use a fixed pass cutoff and `FOR UPDATE SKIP LOCKED`; lease expiry uses live database time. Three workers acquire different Seasons via conditional lease upsert, then process each Season's own target and direct Episodes in pages of 200. Renew the five-minute Season lease and unfinished page targets together. All provider/image work stays outside result transactions. Same-Season selection materializes target IDs before primary-key queue probes; verify both global lookup and page generic plans. The global lookup's job lock only skips contended rows; the conditional Season upsert owns mutual exclusion.
...
- Freeze the cutoff after local reconciliation so newly materialized jobs participate, but retries scheduled during provider work wait for a later execution. No new scheduler or perpetual retry loop. Successful-but-incomplete/404 cooldowns remain unchanged.
...
Wrong: rerun `ListTMDbSeasonMetadataRecheckAfter` over the full catalog on every maintenance execution, disable transaction triggers while saving a recheck result, or globally serialize all Series to hide shared-row contention.

Correct: transactional change registration → bounded expansion → indexed due claim → Series-keyed result serialization → single-target validation → conditional result commit that consumes only its newly registered ordinary target change.
