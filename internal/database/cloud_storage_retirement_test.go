...
func TestRemoveCloudStorageSchemaPreservesLocalAndSharedData(t *testing.T) {
	...
	roots := []model.LibraryRoot{
		...
		{Base: model.Base{ID: "mixed-local-root-2", CreatedAt: time.Now().Add(time.Minute)}, LibraryID: "mixed-lib", Path: "/mixed-second", Enabled: true, SortOrder: 1},
		{Base: model.Base{ID: "mixed-local-root-1", CreatedAt: time.Now()}, LibraryID: "mixed-lib", Path: "/mixed-first", Enabled: true, SortOrder: 0},
	}
	for i := range roots {
	...
	media := []model.Media{
		...
		{Base: model.Base{ID: "mixed-local-media"}, LibraryID: "mixed-lib", LibraryRootID: "mixed-local-root-1", MetadataID: metadata.ID, Path: "/mixed-first/Movie.mkv"},
	}
	for i := range media {
	...
	strmRows := []model.STRMRecord{
		{Base: model.Base{ID: "provider-strm"}, Title: "Cloud", URL: "https://provider.invalid/file", FilePath: "/strm/cloud.strm", Protocol: "OpenList", MediaID: "cloud-media"},
	...
	assertUnscopedCount(t, db, &model.Media{}, "id IN ?", []string{"cloud-media", "mixed-cloud-media"}, 0)
...
func TestRemoveCloudStorageSchemaRollsBackUnsafeMixedLibrary(t *testing.T) {
	...
	assertUnscopedCount(t, db, &model.Media{}, "id = ?", "local-media", 1)
