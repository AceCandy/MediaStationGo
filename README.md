...
| CI/CD | GitHub Actions / Docker Hub | 仅版本标签触发多架构镜像与 Release 包发布 |
...
services:
  mediastation-go:
    image: shukbet/mediastationgo:${MEDIASTATION_IMAGE_TAG:-latest}
    pull_policy: always
...
以后统一使用 Docker Hub 镜像 `shukbet/mediastationgo`；旧的 `ghcr.io/shukebta/mediastation-go` / `mediastationgo` 包可以删除或归档，避免用户拉错镜像。
