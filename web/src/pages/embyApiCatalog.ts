...
export const EMBY_API_ENDPOINTS: readonly EmbyApiEndpoint[] = [
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'SearchTerm', location: 'query', type: 'string', description: '仅搜索顶层 Movie/Series 的标题与原名；按普通分隔符拆词，所有关键词均需命中，LIKE 特殊字符按字面量匹配；Season/Episode 不参与搜索。' },
      ...
      { name: 'Limit / StartIndex', location: 'query', type: 'number', description: '按顶层 Metadata 分页，Limit 默认 50，最大 500。' },
    ],
  ...
  },
  {
    ...
    parameters: [
      tokenHeader,
      { name: 'SearchTerm', location: 'query', type: 'string', required: true, description: '仅搜索顶层 Movie/Series 的标题与原名；按普通分隔符拆词，所有关键词均需命中，LIKE 特殊字符按字面量匹配；Season/Episode 不参与搜索，为空时返回空数组。' },
      { name: 'IncludeItemTypes', location: 'query', type: 'string', description: '搜索时仅 Movie/Series 生效；只请求 Season/Episode 时返回空结果。' },
      { name: 'Limit / StartIndex', location: 'query', type: 'number', description: '按顶层 Metadata 分页。' },
    ],
    responses: [{
      status: '200',
      contentType: 'application/json',
