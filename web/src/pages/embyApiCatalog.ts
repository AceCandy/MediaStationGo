...
export const EMBY_API_ENDPOINTS: readonly EmbyApiEndpoint[] = [
  ...
  },
  {
    ...
    parameters: [
      tokenHeader,
      { name: 'SearchTerm', location: 'query', type: 'string', required: true, description: '搜索词；为空时返回空数组。' },
      { name: 'IncludeItemTypes', location: 'query', type: 'string', description: '逗号分隔的媒体类型。' },
      { name: 'Limit / StartIndex', location: 'query', type: 'number', description: '分页参数。' },
    ],
    responses: [{
      status: '200',
      contentType: 'application/json',
